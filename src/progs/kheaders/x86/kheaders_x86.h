...
struct ip_options {
	...
	unsigned char              is_strictroute:1;     /*    12: 0  1 */
	unsigned char              srr_is_hit:1;         /*    12: 1  1 */
	unsigned char              is_changed:1;         /*    12: 2  1 */
	unsigned char              rr_needaddr:1;        /*    12: 3  1 */
	unsigned char              ts_needtime:1;        /*    12: 4  1 */
	unsigned char              ts_needaddr:1;        /*    12: 5  1 */
...
struct inet_cork {
	...
	__be32                     addr;                 /*     4     4 */
	struct ip_options *        opt;                  /*     8     8 */
	unsigned int               fragsize;             /*    16     4 */
	int                        length;               /*    20     4 */
	struct dst_entry *         dst;                  /*    24     8 */
	u8                         tx_flags;             /*    32     1 */
	__u8                       ttl;                  /*    33     1 */
	__s16                      tos;                  /*    34     2 */
	char                       priority;             /*    36     1 */
...
struct flowi_common {
	int                        flowic_oif;           /*     0     4 */
	int                        flowic_iif;           /*     4     4 */
	int                        flowic_l3mdev;        /*     8     4 */
	__u32                      flowic_mark;          /*    12     4 */
	__u8                       flowic_tos;           /*    16     1 */
	__u8                       flowic_scope;         /*    17     1 */
	__u8                       flowic_proto;         /*    18     1 */
	__u8                       flowic_flags;         /*    19     1 */
	__u32                      flowic_secid;         /*    20     4 */
	kuid_t                     flowic_uid;           /*    24     4 */
	__u32                      flowic_multipath_hash; /*    28     4 */
	struct flowi_tunnel        flowic_tun_key;       /*    32     8 */
...
struct flowi4 {
	struct flowi_common        __fl_common;          /*     0    40 */
	__be32                     saddr;                /*    40     4 */
	__be32                     daddr;                /*    44     4 */
	union flowi_uli            uli;                  /*    48     4 */
...
struct flowi6 {
	struct flowi_common        __fl_common;          /*     0    40 */
	struct in6_addr            daddr;                /*    40    16 */
	struct in6_addr            saddr;                /*    56    16 */
	/* --- cacheline 1 boundary (64 bytes) was 8 bytes ago --- */
	__be32                     flowlabel;            /*    72     4 */
	union flowi_uli            uli;                  /*    76     4 */
	__u32                      mp_hash;              /*    80     4 */
...
struct flowi {
	union {
		struct flowi_common __fl_common;         /*     0    40 */
		struct flowi4      ip4;                  /*     0    56 */
		struct flowi6      ip6;                  /*     0    88 */
	} u;                                             /*     0    88 */
...
struct inet_cork_full {
	struct inet_cork           base;                 /*     0    56 */

	/* XXX last struct has 4 bytes of padding */

	struct flowi               fl;                   /*    56    88 */
...
struct inet_sock {
	struct sock                sk;                   /*     0   760 */
	/* --- cacheline 11 boundary (704 bytes) was 56 bytes ago --- */
	struct ipv6_pinfo *        pinet6;               /*   760     8 */
	/* --- cacheline 12 boundary (768 bytes) --- */
	long unsigned int          inet_flags;           /*   768     8 */
	__be32                     inet_saddr;           /*   776     4 */
	__s16                      uc_ttl;               /*   780     2 */
	__be16                     inet_sport;           /*   782     2 */
	...
	__u8                       pmtudisc;             /*   799     1 */
	__u8                       rcv_tos;              /*   800     1 */
	__u8                       convert_csum;         /*   801     1 */

	/* XXX 2 bytes hole, try to pack */

	int                        uc_index;             /*   804     4 */
	int                        mc_index;             /*   808     4 */
	__be32                     mc_addr;              /*   812     4 */
	u32                        local_port_range;     /*   816     4 */

	/* XXX 4 bytes hole, try to pack */

	struct ip_mc_socklist *    mc_list;              /*   824     8 */
	/* --- cacheline 13 boundary (832 bytes) --- */
	struct inet_cork_full      cork;                 /*   832   144 */

	/* size: 976, cachelines: 16, members: 20 */
	/* sum members: 970, holes: 2, sum holes: 6 */
	/* last cacheline: 16 bytes */
};

struct fastopen_queue {
	struct request_sock *      rskq_rst_head;        /*     0     8 */
	struct request_sock *      rskq_rst_tail;        /*     8     8 */
	spinlock_t                 lock;                 /*    16     4 */
	int                        qlen;                 /*    20     4 */
	int                        max_qlen;             /*    24     4 */

	/* XXX 4 bytes hole, try to pack */

	struct tcp_fastopen_context * ctx;               /*    32     8 */
...
struct request_sock_queue {
	spinlock_t                 rskq_lock;            /*     0     4 */
	u8                         rskq_defer_accept;    /*     4     1 */

	/* XXX 3 bytes hole, try to pack */

	u32                        synflood_warned;      /*     8     4 */
	atomic_t                   qlen;                 /*    12     4 */
	atomic_t                   young;                /*    16     4 */

	/* XXX 4 bytes hole, try to pack */

	struct request_sock *      rskq_accept_head;     /*    24     8 */
	struct request_sock *      rskq_accept_tail;     /*    32     8 */
	struct fastopen_queue      fastopenq;            /*    40    40 */
...
struct tcp_options_received {
	int                        ts_recent_stamp;      /*     0     4 */
	u32                        ts_recent;            /*     4     4 */
	u32                        rcv_tsval;            /*     8     4 */
	u32                        rcv_tsecr;            /*    12     4 */
	u16                        saw_tstamp:1;         /*    16: 0  2 */
	u16                        tstamp_ok:1;          /*    16: 1  2 */
	u16                        dsack:1;              /*    16: 2  2 */
	u16                        wscale_ok:1;          /*    16: 3  2 */
	u16                        sack_ok:3;            /*    16: 4  2 */
	u16                        smc_ok:1;             /*    16: 7  2 */
	u16                        snd_wscale:4;         /*    16: 8  2 */
	u16                        rcv_wscale:4;         /*    16:12  2 */
	u8                         saw_unknown:1;        /*    18: 0  1 */
	u8                         unused:7;             /*    18: 1  1 */
	u8                         num_sacks;            /*    19     1 */
	u16                        user_mss;             /*    20     2 */
	u16                        mss_clamp;            /*    22     2 */

	/* size: 24, cachelines: 1, members: 17 */
	/* last cacheline: 24 bytes */
};
...
struct tcp_rack {
	...
	u32                        rtt_us;               /*     8     4 */
	u32                        end_seq;              /*    12     4 */
	u32                        last_delivered;       /*    16     4 */
	u8                         reo_wnd_steps;        /*    20     1 */
	u8                         reo_wnd_persist:5;    /*    21: 0  1 */
	u8                         dsack_seen:1;         /*    21: 5  1 */
	u8                         advanced:1;           /*    21: 6  1 */
...
struct dql {
	...
	unsigned int               limit __attribute__((__aligned__(64))); /*    64     4 */
	...
	long unsigned int          slack_start_time;     /*    88     8 */
	unsigned int               max_limit;            /*    96     4 */
	unsigned int               min_limit;            /*   100     4 */
	unsigned int               slack_hold_time;      /*   104     4 */

	/* size: 128, cachelines: 2, members: 13 */
	/* sum members: 56, holes: 1, sum holes: 52 */
	/* padding: 20 */
...
struct dev_pm_info {
	...
	unsigned int               can_wakeup:1;         /*     4: 0  4 */
	unsigned int               async_suspend:1;      /*     4: 1  4 */
	...
	bool                       wakeup_path:1;        /*    72: 0  1 */
	bool                       syscore:1;            /*    72: 1  1 */
	bool                       no_pm_callbacks:1;    /*    72: 2  1 */
	bool                       async_in_progress:1;  /*    72: 3  1 */
	unsigned int               must_resume:1;        /*    72: 4  4 */
	unsigned int               may_skip_resume:1;    /*    72: 5  4 */
	...
	unsigned int               disable_depth:3;      /*   224: 0  4 */
	unsigned int               idle_notification:1;  /*   224: 3  4 */
	unsigned int               request_pending:1;    /*   224: 4  4 */
	unsigned int               deferred_resume:1;    /*   224: 5  4 */
	unsigned int               needs_force_resume:1; /*   224: 6  4 */
	unsigned int               runtime_auto:1;       /*   224: 7  4 */
	...
	unsigned int               no_callbacks:1;       /*   224: 9  4 */
	unsigned int               irq_safe:1;           /*   224:10  4 */
	unsigned int               use_autosuspend:1;    /*   224:11  4 */
	unsigned int               timer_autosuspends:1; /*   224:12  4 */
	unsigned int               memalloc_noio:1;      /*   224:13  4 */
	...
	/* size: 312, cachelines: 5, members: 54 */
	/* sum members: 292, holes: 2, sum holes: 8 */
	/* sum bitfield members: 30 bits, bit holes: 3, sum bit holes: 66 bits */
	/* paddings: 1, sum paddings: 4 */
	/* last cacheline: 56 bytes */
};
...
struct device {
	...
	struct io_tlb_mem *        dma_io_tlb_mem;       /*   632     8 */
	/* --- cacheline 10 boundary (640 bytes) --- */
	struct list_head           dma_io_tlb_pools;     /*   640    16 */
	spinlock_t                 dma_io_tlb_lock;      /*   656     4 */
	bool                       dma_uses_io_tlb;      /*   660     1 */
	struct dev_archdata        archdata;             /*   661     0 */

	/* XXX 3 bytes hole, try to pack */

	struct device_node *       of_node;              /*   664     8 */
	struct fwnode_handle *     fwnode;               /*   672     8 */
	int                        numa_node;            /*   680     4 */
	dev_t                      devt;                 /*   684     4 */
	u32                        id;                   /*   688     4 */
	spinlock_t                 devres_lock;          /*   692     4 */
	struct list_head           devres_head;          /*   696    16 */
	/* --- cacheline 11 boundary (704 bytes) was 8 bytes ago --- */
	const struct class  *      class;                /*   712     8 */
	const struct attribute_group  * * groups;        /*   720     8 */
	void                       (*release)(struct device *); /*   728     8 */
	struct iommu_group *       iommu_group;          /*   736     8 */
	struct dev_iommu *         iommu;                /*   744     8 */
	struct device_physical_location * physical_location; /*   752     8 */
	...
	bool                       can_match:1;          /*   764: 4  1 */

	/* size: 768, cachelines: 12, members: 47 */
	/* sum members: 761, holes: 1, sum holes: 3 */
...
struct gnet_stats_basic_sync {
	u64_stats_t                bytes;                /*     0     8 */
	u64_stats_t                packets;              /*     8     8 */
